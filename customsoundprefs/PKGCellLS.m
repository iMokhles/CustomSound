...
NSString* ApplicationStartsPlayingRingtoneLS = @"ApplicationStartsPlayingRingtoneLS";
NSString* ApplicationEndPlayingRingtoneLS = @"ApplicationEndPlayingRingtoneLS";
NSString* ApplicationSuccessfullyPlayingRingtoneLS = @"ApplicationSuccessfullyPlayingRingtoneLS";
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(successfullyFinishPlaying:) name:ApplicationSuccessfullyPlayingRingtoneLS object:nil];
...
-(void)PlayRingtonBtnPressed:(id)sender  {
    if(importBtn.tag == 0)  {
        [importBtn setImage:[UIImage imageNamed:@"pause.png"] forState:UIControlStateNormal];
        importBtn.tag =1;
        [NSNotificationCenter.defaultCenter postNotificationName:ApplicationStartsPlayingRingtoneLS object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:self.textLabel.text, @"fileName", [NSNumber numberWithInteger:self.tag], @"tag", nil]];
    }
    else if(importBtn.tag == 1)   {
        [importBtn setImage:[UIImage imageNamed:@"play.png"] forState:UIControlStateNormal];
        importBtn.tag =0;
        [NSNotificationCenter.defaultCenter postNotificationName:ApplicationEndPlayingRingtoneLS object:nil userInfo:nil];
    }
}
