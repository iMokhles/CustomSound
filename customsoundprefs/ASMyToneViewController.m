...
NSString* ApplicationDidFinishRingtoneConversion = @"ApplicationDidFinishRingtoneConversion";
NSString * ApplicationDidStartPlayingSong = @"ApplicationDidStartPlayingSong";

@interface ASMyToneViewController (){
...
@implementation ASMyToneViewController
...
- (void)viewDidLoad {
    ...
    // if (self.audioPlot == nil && self.startConvertingButton == nil && self.audioPlayBtn == nil && self.ringtoneStartTimeLbl == nil && sel.ringtoneendTimeLbl == nil && self.ringtoneStartTimeLblString == nil && _ringtoneendTimeLblString == nil && _startTimeLbl == nil && _endTimeLbl == nil && _SongNameLbl == nil && _ringtoneNameLbl == nil && _convertingLbl == nil && _startConvertingButton == nil && _audioPlayBtn == nil && _seperatorLbl == nil && _timerSeeker == nil && _ringtoneProgressView == nil) {
	    ...
	    self.ringtoneendTimeLbl.text = @"00:30";
     ...
     */
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(startPlayingRingtone:) name:ApplicationStartsPlayingRingtone object:nil];
...
- (void)openConvertedSongs {
	ASConvertedTonesViewController *convertedSongs = [[ASConvertedTonesViewController alloc] init];
...
-(void) resettingAllControls    {
    ...
    ringtoneendTimeLbl.text = [NSString stringWithFormat:@"%02d:%02d", 0, 30];
...
-(NSString*)getTempMediaFileURL    {
    ...
    libraryCachesDirectory = [@"/var/mobile/Library/CustomSound/Ringtones/" stringByAppendingPathComponent:@"Caches"];
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    if([[textField text] isEqualToString:@""])  {
        ...
        self.ringtoneNameLbl.text = [NSString stringWithFormat:@"%@.m4r", newRingtoneName];
...
-(IBAction)playBtnPressed:(id)sender    {
    ...
    if(audioPlayBtn.tag == 0)   {
        ...
        [NSNotificationCenter.defaultCenter postNotificationName:ApplicationDidStartPlayingSong object:nil userInfo:nil];
    }
    else    {
        if(audioPlayerTimer)    {
            [audioPlayerTimer invalidate];
...
- (void)updateTime:(NSTimer *)timer {
    ...
    int endSeconds = ((seconds + 30)>=60)?((seconds + 30)%60):(seconds + 30);
    int endMinutes = ((seconds + 30)>=60)?(minutes+1):(minutes);
...
//    float endTrimTime = 30;
    ...
    float endTrimTime = (minutes*60+seconds) + 30;
...
{
	ASMyToneViewController *THIS = (__bridge ASMyToneViewController *)inClientData;
...
-(void)convertAudioToRingtoneWithURL:(NSString*) url   {
    ...
    audioConverter = [[TPAACAudioConverter alloc] initWithDelegate:self
                                                             source: url
                                                        destination:[@"/var/mobile/Library/CustomSound/Ringtones/" stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.m4r", self.newRingtoneName]]];
...
-(void)AACAudioConverterDidFinishConversion:(TPAACAudioConverter *)converter {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,  1* NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        [self removeLoadingSpinner];
        [NSNotificationCenter.defaultCenter postNotificationName:ApplicationDidFinishRingtoneConversion object:nil userInfo:nil];
