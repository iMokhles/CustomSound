...
@interface ASConvertedTonesViewControllerLS ()
...
@implementation ASConvertedTonesViewControllerLS
...
-(void) dealloc {
   [NSNotificationCenter.defaultCenter removeObserver:self name:ApplicationEndPlayingRingtoneLS object:nil];
    [NSNotificationCenter.defaultCenter removeObserver:self name:ApplicationStartsPlayingRingtoneLS object:nil];
    [NSNotificationCenter.defaultCenter removeObserver:self name:ApplicationDidFinishRingtoneConversionLS object:nil];
}
...
- (void)viewDidLoad
{
    ...
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(stopPlaying:) name:ApplicationEndPlayingRingtoneLS object:nil];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(startPlaying:) name:ApplicationStartsPlayingRingtoneLS object:nil];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(stopPlayingRingtone:) name:ApplicationDidStartPlayingSongLS object:nil];
    
     [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(successfullyFinishRingtoneConversion:) name:ApplicationDidFinishRingtoneConversionLS object:nil];
}
...
- (void)applySelection {
    ...
    for (NSString *song in self.myRingtonestableviewDatasource) {
        NSString *path = [@"/var/mobile/Library/CustomSound/Sounds/" stringByAppendingPathComponent:[NSString stringWithFormat:@"%@",  song]];
        MDAudioFile *audioFile = [[MDAudioFile alloc] initWithPath:[NSURL fileURLWithPath:path]];
        [songs addObject:audioFile];
    }
    
    MDAudioPlayerController *audioPlayer = [[MDAudioPlayerController alloc] initWithSoundFiles:songs atPath:@"/var/mobile/Library/CustomSound/Sounds/" andSelectedIndex:nil];
...
-(void) stopPlayingRingtone:(NSNotification*) notification  {
    ...
    [NSNotificationCenter.defaultCenter postNotificationName:ApplicationSuccessfullyPlayingRingtoneLS object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:filName, @"fileName", nil]];
    if ( audioPlayer ) {
        [audioPlayer stop];
        audioPlayer = nil;
    }
...
-(void) startPlaying:(NSNotification*) notification {
    ...
    if(playerCellTag != -1) {
        PKGCellLS* cell = (PKGCellLS*)[self.convertedTonesTableview cellForRowAtIndexPath:[NSIndexPath indexPathForRow:playerCellTag inSection:0]];
...
-(void)reloadtableData  {
    ...
    myRingtonestableviewDatasource = [NSMutableArray arrayWithArray:[self findFiles:@"caf"]];
}

-(NSArray *)findFiles:(NSString *)extension{
    
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES);
    NSString *libraryDirectory = @"/var/mobile/Library/CustomSound/Sounds/";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath  {
    ...
    PKGCellLS *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil)   {
        cell = [[PKGCellLS alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
    }
    cell.tag = indexPath.row;
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        NSString *path = [@"/var/mobile/Library/CustomSound/Sounds/" stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", [self.myRingtonestableviewDatasource objectAtIndex:indexPath.row]]];
        didDeleteCell = YES;
...
- (void)playSelectedSong {
    NSIndexPath* selectedRowIndexPath = [self.convertedTonesTableview indexPathForSelectedRow];
    [self.convertedTonesTableview deselectRowAtIndexPath:selectedRowIndexPath animated:YES];
    NSString *path = [@"/var/mobile/Library/CustomSound/Sounds/" stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", [self.myRingtonestableviewDatasource objectAtIndex:selectedRowIndexPath.row]]];
...
-(void)sharingRingtoneViaiTunes {
    ...
    NSString *destinationPath = [@"/var/mobile/Library/CustomSound/ActiveSound/" stringByAppendingPathComponent:@"sound.caf"];
